...
- **The prompt still asks for one thing: keep the heading**, with one exception. A story the baseline has keeps its heading word for word, except where the posts have overtaken it — a figure that has risen, a result now known. Without the exception a heading freezes: measured over four refreshes, the body walked 13 to 15 millions while the heading stayed at `Le ZEVENT franchit les 13 millions d'euros`. Heading stability is what gives `mark_against` something to match on, so nothing else in the line may move. Measured on 600 frozen posts: with the rule, 10 of 10 headings came back word for word; with the rule taken out, 2, and only 5 of 10 still matched at 0.5 — half the page rebuilt and re-badged. The list gives the heading on its own line with the gist indented under it, because a heading can contain a dash.
- **The list is built from the briefing on screen, not the one last read.** `onscreen` in `main.rs` is the newest briefing inside the window; `baseline` still decides the marks. They used to be the same value, and a rename in a run you never opened was handed back to the model as the old heading on the next run and copied back — seen in the log on 6 September, `Le nounours et le klaxon de Sylvain` reverting to `Le nounours de Sylvain` one run later.
- **A heading that has outlived its subject cannot be prompted away.** The nounours section kept its name for two days while its body moved to another gag. Four wordings were tried — same event, name what the section says, name what the posts say, and a one-line rule beside the no-"and" rule — over five runs including one at medium effort. Not one renamed it, and the rule-list wording did the opposite, pulling the dead subject back into the body to keep the heading true. Two mechanical tests were measured and rejected too: heading words absent from the window flag a brand new story just as hard (3 of 7 for `Les smart TV LG espionnent les réseaux locaux`), and body drift since the heading was coined puts a well-named section lower than the stale one (Astra 0.15 against nounours 0.25). What the app does instead is make the rename cheap when the model does make one.
- **A rewritten heading is matched by its wording, not dropped.** Both sides fall back to word overlap at `RENAMED_STORY` / `renamedStory` (0.5, lowercased, order ignored) when no heading matches exactly: the CLI marks the story `[updated]` instead of `[new]`, and the app replaces the block instead of showing the same story twice with two different figures. `Le ZEVENT franchit les 13/15 millions d'euros` scores 0.86; `Le ZEvent critiqué pour ses invités polémiques` against it scores 0.29. Each baseline story can be claimed once, so two similar headings do not both match it.
...
- **The badges only exist in the stored briefing.** Marking happens after generation, so what streams to the screen is unmarked; `adoptMarks` reads the briefing back when the run ends. It goes through `Markdown.inherit` as well, or the badges would land on ids the screen no longer uses and silently never appear; the guard is that the headings match, which is what proves it is the briefing on screen.
...
The three baselines are different questions and must not be merged. `previous` — the last briefing **written**, and wide enough — decides how much has arrived and whether regenerating is worth it. `baseline` — the last briefing **read** — decides what gets marked. `onscreen` — the newest briefing inside the window — is the list the prompt carries, because those are the headings the app has drawn.
...
- **A story's identity is the block on screen, not its heading.** `Markdown.inherit` hands each arriving story the id of the one it replaces — an unchanged heading first, then word overlap at `renamedStory` — so a rewritten heading changes the text in place instead of tearing the section down and inserting a new one further up the page. Each block can be claimed once. Without it a renamed story left its slot, landed wherever the new ordering put it, and came back badged NEW. The heading is still what the match is made on, which is why the prompt still asks for it to be reused: measured 8 of 10 matched exactly after asking, 0 of 10 before.
